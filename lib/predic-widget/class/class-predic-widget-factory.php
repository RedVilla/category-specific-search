...
class Predic_Widget_Factory extends WP_Widget
{
    ...
    public function __construct(array $atts)
    {
        ...
        $pairs = array(
            ...
            'widget_ops' => array(
                ...
                'description' => esc_html__('Some description for example widget.', 'cssfw-lang'),
        ...
        if (empty($this->atts['callback'])) {
            trigger_error(esc_html__('Please provide setup param: callback.', 'cssfw-lang'), E_USER_ERROR);
            return false;
        }

        if (empty($this->atts['base_id']) || empty($this->atts['name'])) {
            trigger_error(esc_html__('Please provide setup params: id and name.', 'cssfw-lang'), E_USER_ERROR);
            return false;
        }
        ...
        /**
         * Add admin scripts for all fields that have method cssfw_admin_scripts and are used in this widget
         */
        $this->fields_cssfw_admin_scripts();
    }
    ...
    public function form($instance)
    {
        ...
        foreach ($this->form_fields as $name => $field) {
            ...
            if (is_int($name)) {
                trigger_error(strip_tags(sprintf(__('You should not use numbers as widget admin form field id. Id provided is: %d.', 'cssfw-lang'), intval($name))), E_USER_NOTICE);
            }
            ...
            if (file_exists($class_path)) {
                ...
                trigger_error(strip_tags(sprintf(__('File %s does not exists.', 'cssfw-lang'), $class_path)), E_USER_WARNING);
                continue;
            }
            ...
            $html .= '<br/>Suggest updates all style';
    ...
    /**
     * Add admin scripts for all fields that have method cssfw_admin_scripts
     * and are used in this widget
     *
     * @since 1.0.1
     */
    public function fields_cssfw_admin_scripts()
    {

        $method = 'cssfw_admin_scripts';
        ...
        foreach ($this->field_php_classes as $class) {
            ...
            if (file_exists($class['path'])) {
                ...
                trigger_error(strip_tags(sprintf(__('File %s does not exists.', 'cssfw-lang'), $class['path'])), E_USER_WARNING);
                continue;
            }
